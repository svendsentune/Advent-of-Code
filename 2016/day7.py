...
def isTLS(inputs):
  ip_list = [l for l in inputs.split('\n')]
  c = 0
  for ip in ip_list:
    hypernets = re.findall(r'\[([a-z]+)\]',ip)
    if any([isABBA(h) for h in hypernets]):
      continue
    else:
      supernets = re.sub(r'\[[a-z]+\]'," ",ip).split(' ')
      if any([isABBA(sn) for sn in supernets]):
        c+=1
  return c

def isABBA(string):
  for i,v in enumerate(string[:-3]):
    if string[i:i+2] == string[i+2:i+4][::-1] and v != string[i+1]:
      return True
  return False
...
def isSSL(inputs):
  ...
  for ip in ip_list:
    ...
    bab_list = []
    for sn in supernets:
      bab_list.append(isABA(sn))
    for bab in list(itertools.chain.from_iterable(bab_list)):
      for h in hypernets:
        if bab in h:
          valid = True
    if valid:
...
def isABA(string):
  bab_list = []
  for i,v in enumerate(string[:-2]):
    if string[i] != string[i+1] and string[i] == string[i+2]:
      bab_list.append(string[i+1]+string[i]+string[i+1])
  return bab_list

print(isSSL('aaaba[kek]eke[eererer]'))



#unit_test1a = "aaa[rkekq]eke"


final_test1 = """xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
assert isTLS(final_test1) == 115 # Part 1

print(isSSL(final_test1))
