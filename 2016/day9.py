...
def compressSequence(some_string):
  new_string = ''
  seen = []
  is_marker = False
  for i in range(len(some_string)):
    if is_marker == True:
      if some_string[i] == ")":
        end_i = i
        span = int(marker.split('x')[0])
        repeat = int(marker.split('x')[1])
        insert  = some_string[i+1:i+1+span]*repeat
        new_string+=insert
        seen.append(i)
        is_marker = False
