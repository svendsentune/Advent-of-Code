...
def checkRoom(input_string):
  ...
  for c in set(encryption):
    if encryption.count(c) not in c_dict:
      c_dict[encryption.count(c)] = [c]
    else:
      c_dict[encryption.count(c)].append(c)
  checksum = ''
...
def sumOfValidRooms(inputs):
  ...
  for r in room_list:
    if checkRoom(r):
      sum_real_rooms += getSectorID(r)
  return sum_real_rooms

def createAlphaDict():
  zipped = zip(list(range(1,27)),list(string.ascii_lowercase))
  alpha_dict = {i[1]:i[0] for i in zipped}
  return alpha_dict



def getNorthPole(inputs):
  room_list = [l for l in inputs.split('\n')]
  for r in room_list:
    sectorID = getSectorID(r)
    name = getEncryptedName(r,dash_to_space=True)
    cypher_dict = createAlphaDict()
    alphabet = list(string.ascii_lowercase)
    decrypted_name = ''
    for c in name:
      if c == " ":
        decrypted_name+=c
      elif cypher_dict[c]+sectorID <= 26:
        decrypted_name+=c
      else:
        decrypted_name += alphabet[(cypher_dict[c]+sectorID)%26-1]
    if decrypted_name == 'northpole object storage':
      return sectorID

testdata = """bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
assert checkRoom("aaaaa-bbb-z-y-x-123[abxyz]")
assert checkRoom("a-b-c-d-e-f-g-h-987[abcde]")
assert checkRoom("not-a-real-room-404[oarel]")
assert not checkRoom("totally-real-room-200[decoy]")

print(sumOfValidRooms(testdata)) # part 1

print(getNorthPole(testdata)) # part 2
