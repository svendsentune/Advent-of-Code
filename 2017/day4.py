...
def highEntropyPassphrases(l):
  c = 0
  for pp in text2list(l)[:4]:
    if len(set(pp))==len(pp):
      c+=1
  return c


def highEntropyAnagrams(text):
  l = [[''.join(sorted(s)) for s in row.split()] for row in text.split("\n")]
  c = 0
  for pp in l:
    if len(set(pp))==len(pp):
      c+=1
...
print(highEntropyAnagrams("""bdwdjjo avricm cjbmj ran lmfsom ivsof
