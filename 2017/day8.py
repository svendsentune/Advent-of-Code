...
Apparently there's a module in Python called eval for this sort of task:
...
def getMaxRegister(text):
  instructions = [t.split() for t in text.split('\n')]
  reg_dict = handleInstruction(instructions)[0]
  return max(reg_dict.values())

def getMaxEver(text):
  instructions = [t.split() for t in text.split('\n')]
  max_value_ever = handleInstruction(instructions)[1]
  return max_value_ever

def handleInstruction(instructions):

  reg_dict = {i[0]:0 for i in instructions}
  max_value_ever = 0
  ...
  for instruction in instructions:
    ...
    target_reg_value = reg_dict[instruction[0]]
    change = operations[instruction[1]]
    change_value = int(instruction[2])
    conditional_reg = reg_dict[instruction[4]]
    condition = operations[instruction[5]]
    condition_value = int(instruction[6])

    if condition(conditional_reg,condition_value):
      reg_dict[target_reg] = change(target_reg_value,change_value)
      if reg_dict[target_reg] > max_value_ever:
        max_value_ever = reg_dict[target_reg]
  return reg_dict,max_value_ever
...
print(getMaxRegister(text))
print(getMaxRegister(test))
print(getMaxEver(test))
